...
export class RelayRewardsService {
    ...
    constructor(
        ...
        this.logger.log(
            `Initializing relay rewards service (IS_LIVE: ${this.isLive})`,
        )
    ...
    public async getLastSnapshot(): Promise<RoundSnapshot | undefined> {
        try {
            ...
            if (!result.Error) {
                this.logger.log(
                    `Last-Snapshot: ${messageId ?? 'no-message-id'}`
                )
                ...
                this.logger.error(
                    `Failed fetching Last-Snapshot: ${result.Error}`,
                )
            }
        } catch (error) {
    ...
    public async addScores(stamp: number, scores: AddScoresData): Promise<boolean> {
        if (this.isLive === 'true') {
            try {
                ...
                if (!result.Error) {
                    this.logger.log(
                        `[${stamp}] Add-Scores ${Object.keys(scores).length}: ${messageId ?? 'no-message-id'}`
                    )
            
                    return true
                } else {
                    this.logger.error(
                        `Failed storing ${Object.keys(scores).length} scores for ${stamp}: ${result.Error}`,
                    )
                }
            } catch (error) {
